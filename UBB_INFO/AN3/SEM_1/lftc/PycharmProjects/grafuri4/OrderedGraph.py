...
class OrderedGraphCost:
    ...
    def minimum(self):
            ...
            U = []
            V = []
            for j in range(self.nrVertices()):
                for i in self.outboundEdge(j):
                    U.append(j)
                    V.append(i)
            ...
            for i in range(N):
                ...
                #keep the count of edges for each node
                count[U[i] - 1] += 1

            # Declare queue Q
            Q = deque()
            #iterate over all nodes and insert each in the queue
            for i in range(N):
                if (count[i] == 0):
                    Q.append(i)
            ...
            while len(Q) > 0:
                u = Q.popleft()
                #while the queue is not empty, i pop one element from the queue
                # Traverse node u
                for x in adj[u]:
                    #for any nodes connected to the popped node, i decrease the count
                    count[x] -= 1
                    if (count[x] == 0):
                        Q.append(x)
                    #if the color is less or equal than the parents colour, we increase the colour
